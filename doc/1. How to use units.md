...
So all __non-defaulted__ units begin with a tiny character, and all __defaulted__ units begin with a capital.
The default type is defined by the **OMNI_DEFAULT_TYPE** macro in settings.hh. It equals **double** by default.
...
For technical reasons, literal operators are only implemented for __official__ and __derived__ units. They aren't defined for composed units (like speed units for example).
